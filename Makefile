...
# openstack.org/octavia-operator-bundle:$VERSION and openstack.org/octavia-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= quay.io/$(USER)/octavia-operator
...
force-bump: ## Force bump operator and lib-common dependencies
	for dep in $$(cat go.mod | grep openstack-k8s-operators | grep -vE -- 'indirect|octavia-operator|^replace' | awk '{print $$1}'); do \
		go get $$dep@$(BRANCH) ; \
	done
	for dep in $$(cat api/go.mod | grep openstack-k8s-operators | grep -vE -- 'indirect|octavia-operator|^replace' | awk '{print $$1}'); do \
